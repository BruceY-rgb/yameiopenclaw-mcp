...
# 4.3 国际机票订单采购生单
# 使用 /api/order/createWechatPayOrder 接口
API_INTL_SAVE_ORDER    = "/api/order/createWechatPayOrder"
...
# 出行人接口（Token 鉴权）
# ─────────────────────────────────────────────
API_PASSENGER_SAVE     = "/api/travel/editTourist"     # 创建/编辑出行人
API_PASSENGER_LIST     = "/api/travel/getTouristList"  # 出行人列表
...
class OntuotuApiClient:
    ...
    async def save_passenger(self, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        路径: POST /api/travel/editTourist
        ...
        Args:
            body: 出行人信息，常用字段：
                - realName: 姓名
                - type: 乘客类型（0=成人 1=儿童 2=婴儿）
                - nationality: 国籍二字码
                - idType: 证件类型（0=护照 2=身份证）
                - idCard: 证件号码
                - phone: 手机号
        """
    ...
    async def list_passengers(self, body: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        路径: GET /api/travel/getTouristList
        """
        return await self.request("GET", API_PASSENGER_LIST)
